...
numerical_columns = ['price', 'distance', 'surge_multiplier', 'latitude', 'longitude', 
                     'temperature', 'humidity', 'precipProbability', 'windSpeed']
df[numerical_columns] = df[numerical_columns].apply(lambda col: col.fillna(col.median()), axis=0)
...
# Drop 'short_summary' as they have been used to create new features
df.drop(columns=['datetime', 'short_summary'], inplace=True)
...
df.to_csv('cleaned_rideshare_data_nn.csv', index=False)

print("Data cleaning and preprocessing completed. Saved to 'cleaned_rideshare_data_nn.csv'.")
